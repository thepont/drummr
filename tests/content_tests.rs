...
fn test_industrial_glitch_kit_integrity() {
    ...
    assert_eq!(kit.sounds.len(), 16, "Industrial Glitch should have 16 sounds");
...
fn test_kit_engine_schema_consistency() {
    ...
    for i in 0..16 {
        ...
        assert!(schema.is_some(), "Slot {} should have a schema in Industrial Glitch kit", i);
        
        let s = schema.unwrap();
        // FM should have at least 6 params
        if i == 0 { // Cyber Kick (fm)
            assert!(s.len() >= 6, "FM schema should have at least 6 parameters, got {}", s.len());
        }
    }
}
...
fn test_invalid_engine_type_fallback() {
    ...
    assert_eq!(engine.get_schema(0).unwrap().len(), 6, "Fallback engine should be FM (6 params)");
}
