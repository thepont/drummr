use drummr::kit::{DrumKit, KitEngine, DrumMapping};
use std::fs;
...
fn test_factory_kits_output() {
    ...
    let mappings: Vec<DrumMapping> = (0..16).map(|i| DrumMapping { note: i as u8, slot: i }).collect();
    ...
    for kit_name in kits {
        ...
        let config: DrumKit = toml::from_str(&content).expect(&format!("Could not parse kit {}", kit_name));
        ...
        for slot in 0..16 {
            ...
            assert!(max_abs > 0.001, "Kit '{}' Slot {} is SILENT!", kit_name, slot);
        }
    }
}
