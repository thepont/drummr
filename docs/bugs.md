...
**Status: RESOLVED.** `handle_command` no longer accepts the `event_consumer` / `cmd_consumer` Arc<Mutex<Option<...>>> parameters — they were dead post-handshake. `main.rs` still owns `event_consumer_wrapped` / `cmd_consumer_wrapped` because they drive the initial `start_audio` call, but they are no longer cloned into the WS dispatcher closure and intentionally remain `None` for the rest of the session. SELECT_AUDIO and the audio-recovery task each build fresh ring buffers and pass the new Consumers directly to `start_audio`.
**Original symptom**: `main.rs` takes the original consumers (line 232) leaving `Some` → `None`, then `SELECT_AUDIO` recreates fresh ring buffers (line 587-588) and passes the new consumers directly into `start_audio`. The original wrapped `Option`s are never re-populated. The handler dodged this with `let _ = event_consumer;` to silence the lint, but the data was structurally meaningless.
...
**Status: RESOLVED.** `LOAD_KIT:<name>` now matches both `fs::read_to_string` and `toml::from_str` explicitly. On either failure it logs to stderr and broadcasts `KIT_ERROR:<name>:read failed: <io error>` or `KIT_ERROR:<name>:parse failed: <toml error>`. Success path unchanged. Covered by `test_load_kit_missing_file_broadcasts_error` and `test_load_kit_malformed_toml_broadcasts_error` in `tests/commands_tests.rs`.
**Original symptom**: If `fs::read_to_string(...)` fails (file missing, permission denied), the `if let Ok(content)` falls through, nothing is broadcast, and the UI's "kit list" still shows the old kit selected. No error response is sent.
